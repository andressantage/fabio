<?php
    ...
    if ($_SESSION['nivel'] === 'todas') {
        $categoria = mysqli_real_escape_string($conexion, $_SESSION['categoria']);
        ...
        $categoria = mysqli_real_escape_string($conexion, $_SESSION['categoria']);
        $nivel = mysqli_real_escape_string($conexion, $_SESSION['nivel']);

        $query = "SELECT * FROM `mas` WHERE correo='$correo' AND categoria='$categoria' AND nivel='$nivel';";
        $resultado = mysqli_query($conexion, $query);
...
<script>
        ...
        //se trae segun nivel
        var datosUsuario = <?php echo json_encode(mysqli_fetch_all($resultado, MYSQLI_ASSOC)); ?>; // Convertir los resultados de MySQL a formato JSON
        //console.log(datosUsuario)

        //se trae todas las palabras
        var datos = <?php echo json_encode(mysqli_fetch_all($resultadoCategoria, MYSQLI_ASSOC)); ?>; // Convertir los resultados de MySQL a formato JSON
        //console.log(datos)
        for(let i=0; i<datosUsuario.length;i++){
            <?php
                if(isset($tod)){
                    echo "lista.push(datosUsuario[i].ing)";
                }else{
                    echo "lista.push(datosUsuario[i].palabra)";
                }
            ?>
             //aqui en lista se guardan las palabras en ingles
        }

        for(let i=0; i<datos.length;i++){
            if(lista.includes(datos[i].ing)){
...
<style>
   ...
   <div class="d-flex flex-row justify-content-between">
    ...
    <div class="inicio">
        ...
        <h2 class="text-white mb-0" id="palabraFonetica">&nbsp</h2>

        <!-- imagen mas los dos botones -->
        <div class="medio">
            <button name="boton" class="btn btn-success px-4 item" value="facil" id="facil">FACIL</button>
            <img class="item" id="imagen" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQwPFaudjtLfihsrvL_wGgYVp5g5Jykdgt4fg&usqp=CAU" alt="imagen">
            <button name="boton" class="btn btn-danger item" value="dificil" id="dificil">DIFICIL</button>
        </div>
        <div class="contenidoFrases pt-0">
            <!-- palabra a aprender -->
            <h1 class="frases mt-1" id="palabra">Ninguno</h1> <!-- Better -->
            <!-- frase --><!--  Practice makes perfect, and with each attempt, you become better. -->
            <h2 class="text-white mb-0" id="frase">Seleccionar nivel como Todas e ir definiendo las palabras como te parezca</h2>
        </div>
        <!-- los tres botones -->
        <div class="padding-y">
