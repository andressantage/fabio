...
import { getFirestore, collection, addDoc, query, where, getDocs, updateDoc } from "https://www.gstatic.com/firebasejs/11.3.1/firebase-firestore.js";
...
// Actualizar la interfaz con la palabra actual y opciones
function actualizarInterfaz(userEmail) {
    if (indicePalabra >= dic.length) {
        ...
        audio.src = "";
        opciones.forEach(btn => btn.innerHTML = "");
        next.disabled = true;
        return;
    }

    const palabraActual = dic[indicePalabra];
    imagen.src = palabraActual["Link"] || "https://lamenteesmaravillosa.com/wp-content/uploads/2021/11/rosa-blanca.jpg";
    audio.src = palabraActual["Audio"] || "";
    palabra.innerHTML = " "; // Espacio en blanco inicial

    // Preparar lista de opciones (palabras en inglés)
    let opcionesLista = [palabraActual["Ingles"]];
    let palabrasOtras = [...listaPalabras];
    shuffleArray(palabrasOtras);
    palabrasOtras = palabrasOtras.filter(p => p !== palabraActual["Ingles"]).slice(0, 5); // 5 opciones diferentes
    opcionesLista = opcionesLista.concat(palabrasOtras);
    shuffleArray(opcionesLista);

    // Asignar opciones a los botones
    opciones.forEach((btn, index) => btn.innerHTML = opcionesLista[index]);
    ...
    opciones.forEach(boton => {
        ...
        boton.clickHandler = (event) => {
            const textoClicado = event.target.innerHTML;
            if (textoClicado === palabraActual["Ingles"]) {
...
function inicializarInterfaz(userEmail) {
    ...
    facil.addEventListener("click", () => {
        guardarRespuestaFirebase(dic[indicePalabra]["Ingles"], "facil", userEmail);
        siguientePalabra(userEmail);
    });
    
    dificil.addEventListener("click", () => {
        guardarRespuestaFirebase(dic[indicePalabra]["Ingles"], "dificil", userEmail);
        siguientePalabra(userEmail);
    });
}
